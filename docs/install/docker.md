...
- runtime 데이터를 둘 호스트 경로 — 예: `/volume1/docker/myphotos/data`.
  **미리 `mkdir -p` 로 만들어 두십시오** — Docker 바인드 마운트는
  호스트 경로가 없으면 자동 생성 안 하고 `Bind mount failed: ... does
  not exists` 로 떨어집니다.
...
> ⚠ `DATA_DIR` 경로는 **사전 준비** 단계에서 이미 만들어 두셨어야
> 합니다. 빠뜨리셨다면 `mkdir -p $(grep -E '^DATA_DIR=' .env | cut -d= -f2-)`
> 한 번 실행해 두십시오.
...
- Host path for runtime data — e.g. `/volume1/docker/myphotos/data`.
  **Create it with `mkdir -p` ahead of time** — Docker bind mounts
  don't auto-create the host path, and `compose up` will fail with
  `Bind mount failed: ... does not exists` if it's missing.
...
> ⚠ The `DATA_DIR` path should already exist from the **Prerequisites**
> step. If you skipped it, run
> `mkdir -p $(grep -E '^DATA_DIR=' .env | cut -d= -f2-)` once.
