...
| classify/detect 잡이 `Permission denied: /app/data/models/...onnx` 또는 `ONNXRuntimeError ... system error number 13`로 실패 | ml-worker 로그엔 `model found`라고 떠도 실제 **읽기**에서 막힌 것 — 호스트에서 `sudo`로 받은 모델 파일이 **root 소유**라 컨테이너 UID 1000이 못 엽니다. `sudo chown -R 1000:1000 data/models` (또는 `chmod -R a+rX data/models`) 후 `docker compose restart ml-worker`. 실패한 잡은 관리 → 색인에서 재시도. |
...
| classify/detect jobs fail with `Permission denied: /app/data/models/...onnx` or `ONNXRuntimeError ... system error number 13` | The ml-worker log may say `model found`, but the actual **read** is blocked — models downloaded on the host with `sudo` are **root-owned** and UID 1000 can't open them. `sudo chown -R 1000:1000 data/models` (or `chmod -R a+rX data/models`), then `docker compose restart ml-worker`. Retry the failed jobs from Admin → Indexing. |
