...
- **ML 가속 (GPU)** — ML 워커가 쓸 ONNX 실행 장치를 드롭다운으로 선택:
  ...
  `OpenVINO`(Intel). **자동 모드면 GPU가 감지되는데 onnxruntime가 CPU 빌드일
  때, 앱이 시작 시 프로젝트 venv의 런타임을 GPU용(Windows=`onnxruntime-directml`,
  Linux+NVIDIA=`onnxruntime-gpu`)으로 알아서 교체하고 ML 워커를 재시작합니다**
  (진행 로그 창 표시, 한 번만). 즉 GPU 머신에선 추가 설정 없이 GPU를 씁니다.
  선택은 ML 워커 실행 시 환경변수로 주입되며 `config/local.toml`은 건드리지
  않습니다 — NAS는 CPU, 색인용 PC는 GPU로 같은 체크아웃을 다르게 굴릴 수
  있습니다. **`GPU 확인`** 버튼은 현재 onnxruntime가 어떤 장치를 지원하는지
  보여줍니다. 장치를 바꾸면 ML 워커 재시작 시 적용됩니다(앱이 재시작을 물어봄).
